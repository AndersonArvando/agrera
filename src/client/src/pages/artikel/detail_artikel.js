...
function DetailArtikel() {
    ...
    const handleLikeClick = async (comment_id) => {
        ...
        try {
            const dataResponse = await fetch(`/article/like/${id}`, {
                method: 'POST',
    ...
    const handleSubmitComment = async (e) => {
        e.preventDefault();
        try {
            const dataResponse = await fetch(`/article/comment/${id}`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json', Authorization: token },
    ...
    const fetchData = async () => {
        try {
            const dataResponse = await fetch(`/detail/article/get/${id}`, {
                method: 'GET',
                headers: { 'Content-Type': 'application/json', Authorization: token },
            });
    ...
    return (
        <div>
            ...
            <div className="container" style={{ paddingTop: '6rem', paddingBottom: '6rem' }}>
                <div className="px-4 py-2 rounded-5 mb-2" style={{ border: '1px solid #028430', width: 'fit-content' }}>{article.category_name}</div>
                <h1 className="color-primary mb-3" style={{ fontWeight: '800' }}>
                    {article.title}
                </h1>
                <p className="fs-5">{article.createdAt && new Intl.DateTimeFormat('en-GB', {
                    day: 'numeric',
                    month: 'long',
                    year: 'numeric',
            ...
            <img src={'/' + article.image} className="w-100 mb-4" style={{ aspectRatio: '3/2', objectFit: 'contain' }} />

            <div className="container" style={{ borderBottom: '1px solid #373900' }}>
                <p style={{ textAlign: 'justify' }} className="mb-4" dangerouslySetInnerHTML={{ __html: article.description }}>
                </p>
                {localStorage.getItem('authToken') ?
                    isLikes ? <div onClick={handleLikeClick} className="py-2 px-3 rounded-3 mb-4" style={{ border: '2px solid #0B7077', backgroundColor: '#074E53', width: 'fit-content', cursor: 'pointer' }}>
                        ...
                        <span className="ms-2 mb-4 fw-bold" style={{ color: 'white' }}>LIKE ({article.likes})</span>
                        ...
                        <span className="ms-2 mb-4 fw-bold" style={{ color: '#074E53' }}>LIKE ({article.likes})</span>
                    </div>

                    :
                    <Link to="/login">
                        <div className="py-2 px-3 rounded-3 mb-4" style={{ border: '2px solid #{article.likes}B7077', width: 'fit-content', cursor: 'pointer' }}>
                            <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                                <path d="M20 9C20 8.46957 19.8084 7.96086 19.4675 7.58579C19.1265 7.21071 18.664 7 18.1818 7H12.4364L13.3091 2.43C13.3273 2.33 13.3364 2.22 13.3364 2.11C13.3364 1.7 13.1818 1.32 12.9364 1.05L11.9727 0L5.99091 6.58C5.65455 6.95 5.45455 7.45 5.45455 8V18C5.45455 18.5304 5.6461 19.0391 5.98708 19.4142C6.32805 19.7893 6.79052 20 7.27273 20H15.4545C16.2091 20 16.8545 19.5 17.1273 18.78L19.8727 11.73C19.9545 11.5 20 11.26 20 11V9ZM0 20H3.63636V8H0V20Z" fill="#074E53" />
                            </svg>
                            <span className="ms-2 mb-4 fw-bold" style={{ color: '#074E53' }}>LIKE ({article.likes})</span>
