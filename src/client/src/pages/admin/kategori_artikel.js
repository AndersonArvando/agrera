...
function AdminKategoriArtikel() {
    ...
    const handleDelete = async (id) => {
        ...
        if (confirmDelete) {
            try {
                const response = await fetch(`/admin/category/article/delete/${id}`, {
                    method: "POST",
                });
                ...
                <div className="mb-3">
                    <div className="mb-4 d-flex flex-wrap justify-content-between">
                        <span className="fs-4 " style={{
                            color: '#028430',
                            fontWeight: 'bold',
                        }}>Tabel Data Kategori Artikel</span>
                        <Link to={'/admin/kategori/artikel/add'}>
                        ...
                        <tbody>
                            {
                                data.length > 0 ? data.map((item, index) => (
                                    <tr>
                                        ...
                                        <td>{item.name}</td>
                                        <td>
                                            <Link to={"/admin/kategori/artikel/edit/" + item.id}>
...
export default AdminKategoriArtikel;
